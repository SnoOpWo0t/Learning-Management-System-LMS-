        ...
        const sampleBlogs = [
          {
            ...
            author: contentUser?.id || adminUser?.id,
            publishedAt: new Date()
          },
          {
            ...
            author: adminUser?.id || contentUser?.id,
            publishedAt: new Date()
          },
          {
...
async function seedDemoData(strapi: Core.Strapi) {
  ...
  if (!existingStudent3) {
    ...
    if (course1Res.length > 0) {
      ...
      await strapi.entityService.create('api::enrollment.enrollment', {
        data: { student: s2.id, course: c1.id, publishedAt: new Date() } as any
      });

      const lessons = await strapi.db.query('api::lesson.lesson').findMany({ where: { course: c1.id } });
      for (const l of lessons) {
        await strapi.entityService.create('api::lesson-progress.lesson-progress', {
          data: { student: s2.id, lesson: l.id, completed: true, publishedAt: new Date() } as any
        });
      }

      await strapi.entityService.create('api::quiz-result.quiz-result', {
        data: { student: s2.id, quiz: quiz2.id, score: 100, totalQuestions: 2, publishedAt: new Date() } as any
      });

      // Student 3 (50% Progress, 50% Quiz)
      await strapi.entityService.create('api::enrollment.enrollment', {
        data: { student: s3.id, course: c1.id, publishedAt: new Date() } as any
      });

      await strapi.entityService.create('api::lesson-progress.lesson-progress', {
        data: { student: s3.id, lesson: lessons[0].id, completed: true, publishedAt: new Date() } as any
      });

      await strapi.entityService.create('api::quiz-result.quiz-result', {
        data: { student: s3.id, quiz: quiz2.id, score: 50, totalQuestions: 2, publishedAt: new Date() } as any
      });
